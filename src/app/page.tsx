        ...
        <div className="h-120 px-8 py-10 sm:px-12 sm:py-12">
          <div className="max-w-4xl">
            <div className="mb-4 flex items-center gap-8">
              <div className="w-32 h-48 rounded-lg shadow-2xl border border-black/20" />
              <div className="flex-1">
                <p className="text-[#8B0000] text-sm font-semibold uppercase tracking-[0.16em]">Featured Artist</p>
                <h1 className="text-5xl sm:text-6xl font-semibold tracking-tight text-gray-900 mt-3 mb-5">
                  Featured Artist
                </h1>
                <div className="flex items-center gap-4 mb-5 text-sm">
                  <span className="text-gray-600 font-semibold">Dominican Legend</span>
                  <span className="text-gray-600 font-semibold">Merengue</span>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <Link href="/recordings" className="bg-[#002D62] hover:bg-[#001A3A] text-white font-semibold px-7 py-2.5 rounded-full shadow-sm transition-colors">
                    More Info
                  </Link>
                </div>
              </div>
          ...
          <div className="grid gap-5 grid-cols-2 sm:grid-cols-3 lg:grid-cols-6">
            {[1, 2, 3, 4, 5, 6].map((id) => (
              <Link key={id} href={`/artists/${id}`} className="group">
                ...
                <p className="text-xs text-gray-600 mt-1">Merengue • Dominican</p>
              </Link>
            ))}
          </div>
        </section>
          ...
          <div className="grid gap-5 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
            {[1, 2, 3, 4, 5].map((id) => (
              <Link key={id} href={`/artists/${id}`} className="group">
                ...
                <p className="text-xs text-gray-600 mt-1">Dominican Legend</p>
              </Link>
            ))}
          </div>
        </section>
              ...
              >
                ...
                <div className="space-y-2.5">
                  {category.items.map((item) => (
                    ...
                    >
                      <span className="inline-flex items-center transition-transform group-hover:translate-x-1">→ {item}</span>
                    </Link>
                  ))}
                </div>
              </div>
